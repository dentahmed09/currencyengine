...
# ──── Sidebar ────────────────────────────────────────────────────────────
with st.sidebar:
    st.markdown("**الملفات المستخدمة**")
    st.caption(f"يومي: {DAILY_FILE}")
    st.caption(f"أسبوعي: {WEEKLY_FILE}")
    st.caption(f"شهري: {MONTHLY_FILE}")

    if st.button("🗑️ مسح كل البيانات"):
        for f in [DAILY_FILE, WEEKLY_FILE, MONTHLY_FILE]:
            if os.path.exists(f):
                os.remove(f)
        st.success("تم المسح – أعد التشغيل")
        st.rerun()
